...
class JacoPixelEnv(Env):
    def __init__(self, env, is_rgb=True):
        self._env = env
        self.is_rgb = is_rgb
        if self.is_rgb:
            self._observation_space = Box(low=0.0, high=1.0, shape=(84, 84, 3)) # 42, 42, 3
        else:
            self._observation_space = Box(low=0.0, high=1.0, shape=(84, 84, 1)) # 42, 42, 1
    ...
    def step(self, action):
        ...
        next_frame = self._process_frame84(self._env.render('rgb_array'))
